...
#import "StockKDayModel.h"
#import "StockMinuteModel.h"
...
//    }];
    ...
    [BaseJSON MinuteWithSymbol:@"ZPIN" Success:^(id jsonObj, NSInteger returnCode, NSArray *data) {
        if (returnCode == 0) {
            StockMinuteModel *model = [StockMinuteModel mj_objectWithKeyValues:jsonObj];
            
            NSMutableArray *array = [NSMutableArray array];
            NSArray *minArr = model.data;
            [minArr enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                YYTimeLineModel *model = [[YYTimeLineModel alloc]initWithDict:obj];
    ...
    [BaseJSON KDayWithSymbol:@"AMD" Success:^(id jsonObj, NSInteger returnCode, NSArray *data) {
        if (returnCode == 0) {
            StockKDayModel *model = [StockKDayModel mj_objectWithKeyValues:jsonObj];
            
            NSMutableArray *array = [NSMutableArray array];
            NSArray *dayArr = model.data;
